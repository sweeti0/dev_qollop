      ...
      local get_cmd = extra.get_cmd
       if get_cmd == "طرد" then
         ...
         if is_momod2(member_id, chat_id) and not is_admin2(sender) then
            send_large_msg(receiver, "❗️ لا تستطيع طرد الادمنيه او المدير")
			return
         end
		 kick_user(member_id, chat_id)
      elseif get_cmd == 'حظر' then
        if is_momod2(member_id, chat_id) and not is_admin2(sender) then
			send_large_msg(receiver, "❗️❌لا تستطيع طرد الادمنيه او الدير")
			return
        end
        send_large_msg(receiver, '❗️ العضو@'.member..' ['..member_id..'] تم حظر العضو')
		ban_user(member_id, chat_id)
      elseif get_cmd == 'الغاء الحظر' then
        send_large_msg(receiver, 'العضو @'..member..' ['..member_id..'] تم الغاء الحظر عن العضو')
        local hash =  'banned:'..chat_id
             redis:srem(hash, member_id)
        return 'العضو '..user_id..' تم الغاء الحظر عن العضو'
      elseif get_cmd == 'حظر عام' then
        send_large_msg(receiver, 'العضو @'..member..' ['..member_id..'] تم حظر العضو بشكل عام')
		banall_user(member_id)
      elseif get_cmd == 'الغاء العام' then
        send_large_msg(receiver, 'العضو @'..member..' ['..member_id..'] تم الغاء الحظر عن العضو بشكل عام')
  ...
  if matches[1]:lower() == 'حظر' then-- /ban
    ...
    elseif string.match(matches[2], '^%d+$') then
        ...
        if not is_admin1(msg) and is_momod2(matches[2], msg.to.id) then
          	return "❗️❌لا تستطيع طرد الادمنيه والمدير"
        end
		...
		send_large_msg(receiver, 'العضو ['..matches[2]..'] تم الحظر العضو')
      else
		local cbres_extra = {
		chat_id = msg.to.id,
		get_cmd = 'حظر',
...
if matches[1]:lower() == 'طرد' then
    ...
    elseif string.match(matches[2], '^%d+$') then
		...
		if not is_admin1(msg) and is_momod2(matches[2], msg.to.id) then
			return "❗️❌لا تستطيع طرد الادمنيه و المدير"
		end
		if tonumber(matches[2]) == tonumber(msg.from.id) then
			return "❗️❌لا تستطيع طرد نفسك"
		end
      ...
      if string.match(targetuser, '^%d+$') then
        if tonumber(matches[2]) == tonumber(our_id) then
        	...
        	banall_user(targetuser)
       		return 'العضو ['..user_id..' ] تم حظر العضو بشكل عام'
     else
	local cbres_extra = {
		chat_id = msg.to.id,
		get_cmd = 'حظر عام',
    ...
    local chat_id = msg.to.id
      if string.match(matches[2], '^%d+$') then
        if tonumber(matches[2]) == tonumber(our_id) then
        	...
        	return 'العضو ['..user_id...'] تم الغاء الحظر ع'
    else
		local cbres_extra = {
			chat_id = msg.to.id,
			get_cmd = 'الغاء العام',
